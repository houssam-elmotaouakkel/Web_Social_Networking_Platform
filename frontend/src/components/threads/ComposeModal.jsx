...
import { X, Image, Globe, Users, Lock, Loader2, ChevronDown, Check } from 'lucide-react'
...
export default function ComposeModal({ isOpen, onClose, onCreated }) {
  ...
  const {
    ...
    handleFileSelect, removeMedia, handleSubmit,
    autoResize, reset, canPost, hasDraft,
  } = composer
  ...
  const handleClose = useCallback(async () => {
    if (hasDraft) {
      if (!(await confirm(t('compose.discardDraft')))) return
    }
    reset()
    onClose()
  }, [hasDraft, t, reset, onClose, confirm])
  ...
  return (
    <>
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-16 px-4" onClick={handleClose}>
      <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" />

      <div
        ref={trapRef}
        className="relative w-full max-w-xl rounded-2xl border border-border bg-bg-card shadow-2xl"
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-border">
          <button
            onClick={handleClose}
            className="p-1.5 rounded-full hover:bg-bg-hover cursor-pointer transition-colors"
            ...
            <div className="flex-1 min-w-0">
              <textarea
                ...
                className="w-full bg-transparent text-text-primary placeholder-text-muted
                           resize-none outline-none text-[15px] leading-relaxed py-1"
              />

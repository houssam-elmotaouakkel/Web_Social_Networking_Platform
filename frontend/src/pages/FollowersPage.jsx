...
export default function FollowersPage() {
  ...
  useDocumentTitle('pageTitle.followers')
  ...
  return (
    <div>
      <div className="sticky top-0 z-10 bg-bg-primary/80 backdrop-blur-xl border-b border-border
        ...
        <div>
          <h2 className="text-xl font-bold text-text-primary">{t('follows.followersTitle')}</h2>
          <p className="text-xs text-text-muted">{count} {t('follows.followers')}</p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <p className="text-center text-text-muted py-12 text-sm">{t('follows.noFollowersYet')}</p>
      ) : (
